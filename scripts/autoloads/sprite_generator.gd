...
# ---- Blade Knight diagonal directional sprites ----

func _gen_blade_knight_dir_down_side() -> void:
	# 3/4 front-diagonal — facing down-right (flip_h for down-left)
	# Body shifted 2px right, visor partially visible, sword/shield at angle
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _bk_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	# Boots — staggered (near foot lower, far foot higher)
	_fill_rect(img, 11, 40, 5, 6, c["boot"])
	_fill_rect(img, 18, 41, 5, 5, c["boot"])
	# Legs
	_fill_rect(img, 12, 32, 4, 9, c["armor_dark"])
	_fill_rect(img, 18, 33, 4, 8, c["armor_dark"])
	# Torso (shifted slightly right for 3/4 perspective)
	_fill_rect(img, 10, 18, 13, 15, c["armor_mid"])
	_fill_rect(img, 11, 19, 11, 13, c["armor_light"])
	_fill_rect(img, 10, 30, 13, 3, c["armor_dark"])
	# Shoulders — far shoulder smaller
	_fill_rect(img, 6, 18, 5, 5, c["armor_mid"])
	_fill_rect(img, 21, 18, 6, 5, c["armor_mid"])
	# Helm (3/4 — visor partially visible)
	_fill_rect(img, 12, 6, 10, 13, c["helm"])
	_fill_rect(img, 13, 7, 8, 11, c["armor_mid"])
	_fill_rect(img, 16, 11, 5, 3, c["visor"])
	_fill_rect(img, 15, 4, 4, 4, c["armor_light"])
	# Sword at angle (resting, more forward)
	_fill_rect(img, 25, 18, 3, 18, c["sword"])
	_fill_rect(img, 26, 16, 2, 4, c["sword_glow"])
	_fill_rect(img, 24, 34, 5, 3, c["armor_dark"])
	# Shield (partially visible, angled toward viewer)
	_fill_rect(img, 3, 21, 6, 9, c["shield"])
	_fill_rect(img, 3, 21, 6, 2, c["shield_rim"])
	_fill_rect(img, 3, 28, 6, 2, c["shield_rim"])
	_fill_rect(img, 3, 21, 2, 9, c["shield_rim"])
	textures["blade_knight_dir_down_side"] = ImageTexture.create_from_image(img)

func _gen_blade_knight_dir_up_side() -> void:
	# 3/4 back-diagonal — facing up-right (flip_h for up-left)
	# Cape visible on one side, back of helm, sword on back at angle
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _bk_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 18, 33, 4, 8, c["armor_dark"])
	# Torso (back, shifted)
	_fill_rect(img, 10, 18, 13, 15, c["armor_dark"])
	_fill_rect(img, 11, 19, 11, 13, c["armor_mid"])
	# Cape on near side
	_fill_rect(img, 7, 22, 5, 16, Color(0.15, 0.2, 0.5))
	_fill_rect(img, 8, 23, 3, 14, Color(0.2, 0.25, 0.55))
	_fill_rect(img, 10, 30, 13, 3, c["armor_dark"])
	# Shoulders
	_fill_rect(img, 6, 18, 5, 5, c["armor_mid"])
	_fill_rect(img, 21, 18, 6, 5, c["armor_mid"])
	# Helm back (3/4)
	_fill_rect(img, 12, 6, 10, 13, c["helm"])
	_fill_rect(img, 13, 7, 8, 11, c["armor_dark"])
	_fill_rect(img, 15, 4, 4, 4, c["armor_light"])
	# Sword across back (angled)
	_fill_rect(img, 15, 8, 3, 26, c["sword"])
	_fill_rect(img, 14, 32, 5, 3, c["armor_dark"])
	# Shield on back (partially visible)
	_fill_rect(img, 20, 14, 5, 7, c["shield"])
	_fill_rect(img, 20, 14, 5, 2, c["shield_rim"])
	textures["blade_knight_dir_up_side"] = ImageTexture.create_from_image(img)

# ---- Blade Knight diagonal walk cycle frames ----

func _gen_blade_knight_walk_down_side_1() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _bk_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	# Left foot forward, right back (diagonal stride)
	_fill_rect(img, 9, 39, 5, 6, c["boot"])
	_fill_rect(img, 20, 41, 5, 5, c["boot"])
	_fill_rect(img, 10, 32, 4, 8, c["armor_dark"])
	_fill_rect(img, 20, 33, 4, 8, c["armor_dark"])
	# Torso (bounce down)
	_fill_rect(img, 10, 19, 13, 15, c["armor_mid"])
	_fill_rect(img, 11, 20, 11, 13, c["armor_light"])
	_fill_rect(img, 10, 31, 13, 3, c["armor_dark"])
	_fill_rect(img, 6, 19, 5, 5, c["armor_mid"])
	_fill_rect(img, 21, 19, 6, 5, c["armor_mid"])
	# Helm
	_fill_rect(img, 12, 7, 10, 13, c["helm"])
	_fill_rect(img, 13, 8, 8, 11, c["armor_mid"])
	_fill_rect(img, 16, 12, 5, 3, c["visor"])
	_fill_rect(img, 15, 5, 4, 4, c["armor_light"])
	# Sword/shield
	_fill_rect(img, 25, 19, 3, 18, c["sword"])
	_fill_rect(img, 24, 35, 5, 3, c["armor_dark"])
	_fill_rect(img, 3, 22, 6, 9, c["shield"])
	_fill_rect(img, 3, 22, 6, 2, c["shield_rim"])
	_fill_rect(img, 3, 29, 6, 2, c["shield_rim"])
	_fill_rect(img, 3, 22, 2, 9, c["shield_rim"])
	textures["blade_knight_walk_down_side_1"] = ImageTexture.create_from_image(img)

func _gen_blade_knight_walk_down_side_2() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _bk_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	# Feet together (passing)
	_fill_rect(img, 13, 40, 5, 6, c["boot"])
	_fill_rect(img, 16, 40, 5, 6, c["boot"])
	_fill_rect(img, 14, 32, 4, 9, c["armor_dark"])
	_fill_rect(img, 16, 32, 4, 9, c["armor_dark"])
	# Torso (up bounce)
	_fill_rect(img, 10, 17, 13, 15, c["armor_mid"])
	_fill_rect(img, 11, 18, 11, 13, c["armor_light"])
	_fill_rect(img, 10, 29, 13, 3, c["armor_dark"])
	_fill_rect(img, 6, 17, 5, 5, c["armor_mid"])
	_fill_rect(img, 21, 17, 6, 5, c["armor_mid"])
	# Helm
	_fill_rect(img, 12, 5, 10, 13, c["helm"])
	_fill_rect(img, 13, 6, 8, 11, c["armor_mid"])
	_fill_rect(img, 16, 10, 5, 3, c["visor"])
	_fill_rect(img, 15, 3, 4, 4, c["armor_light"])
	# Sword/shield
	_fill_rect(img, 25, 17, 3, 18, c["sword"])
	_fill_rect(img, 24, 33, 5, 3, c["armor_dark"])
	_fill_rect(img, 3, 20, 6, 9, c["shield"])
	_fill_rect(img, 3, 20, 6, 2, c["shield_rim"])
	_fill_rect(img, 3, 27, 6, 2, c["shield_rim"])
	_fill_rect(img, 3, 20, 2, 9, c["shield_rim"])
	textures["blade_knight_walk_down_side_2"] = ImageTexture.create_from_image(img)

func _gen_blade_knight_walk_down_side_3() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _bk_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	# Right foot forward, left back (mirror of frame 1)
	_fill_rect(img, 20, 39, 5, 6, c["boot"])
	_fill_rect(img, 9, 41, 5, 5, c["boot"])
	_fill_rect(img, 20, 32, 4, 8, c["armor_dark"])
	_fill_rect(img, 10, 33, 4, 8, c["armor_dark"])
	# Torso (bounce down)
	_fill_rect(img, 10, 19, 13, 15, c["armor_mid"])
	_fill_rect(img, 11, 20, 11, 13, c["armor_light"])
	...
	_fill_rect(img, 3, 29, 6, 2, c["shield_rim"])
	_fill_rect(img, 3, 22, 2, 9, c["shield_rim"])
	textures["blade_knight_walk_down_side_3"] = ImageTexture.create_from_image(img)

func _gen_blade_knight_walk_up_side_1() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _bk_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 10, 32, 4, 8, c["armor_dark"])
	_fill_rect(img, 20, 33, 4, 8, c["armor_dark"])
	# Torso back (bounce down)
	_fill_rect(img, 10, 19, 13, 15, c["armor_dark"])
	_fill_rect(img, 11, 20, 11, 13, c["armor_mid"])
	_fill_rect(img, 7, 23, 5, 16, Color(0.15, 0.2, 0.5))
	_fill_rect(img, 8, 24, 3, 14, Color(0.2, 0.25, 0.55))
	_fill_rect(img, 10, 31, 13, 3, c["armor_dark"])
	_fill_rect(img, 6, 19, 5, 5, c["armor_mid"])
	_fill_rect(img, 21, 19, 6, 5, c["armor_mid"])
	# Helm back
	_fill_rect(img, 12, 7, 10, 13, c["helm"])
	_fill_rect(img, 13, 8, 8, 11, c["armor_dark"])
	_fill_rect(img, 15, 5, 4, 4, c["armor_light"])
	# Sword on back
	_fill_rect(img, 15, 9, 3, 26, c["sword"])
	_fill_rect(img, 14, 33, 5, 3, c["armor_dark"])
	_fill_rect(img, 20, 15, 5, 7, c["shield"])
	_fill_rect(img, 20, 15, 5, 2, c["shield_rim"])
	textures["blade_knight_walk_up_side_1"] = ImageTexture.create_from_image(img)

func _gen_blade_knight_walk_up_side_2() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _bk_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 16, 32, 4, 9, c["armor_dark"])
	# Torso back (up bounce)
	_fill_rect(img, 10, 17, 13, 15, c["armor_dark"])
	_fill_rect(img, 11, 18, 11, 13, c["armor_mid"])
	_fill_rect(img, 7, 21, 5, 16, Color(0.15, 0.2, 0.5))
	_fill_rect(img, 8, 22, 3, 14, Color(0.2, 0.25, 0.55))
	_fill_rect(img, 10, 29, 13, 3, c["armor_dark"])
	_fill_rect(img, 6, 17, 5, 5, c["armor_mid"])
	_fill_rect(img, 21, 17, 6, 5, c["armor_mid"])
	# Helm
	_fill_rect(img, 12, 5, 10, 13, c["helm"])
	_fill_rect(img, 13, 6, 8, 11, c["armor_dark"])
	_fill_rect(img, 15, 3, 4, 4, c["armor_light"])
	# Sword on back
	_fill_rect(img, 15, 7, 3, 26, c["sword"])
	_fill_rect(img, 14, 31, 5, 3, c["armor_dark"])
	_fill_rect(img, 20, 13, 5, 7, c["shield"])
	_fill_rect(img, 20, 13, 5, 2, c["shield_rim"])
	textures["blade_knight_walk_up_side_2"] = ImageTexture.create_from_image(img)

func _gen_blade_knight_walk_up_side_3() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _bk_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 20, 15, 5, 7, c["shield"])
	_fill_rect(img, 20, 15, 5, 2, c["shield_rim"])
	textures["blade_knight_walk_up_side_3"] = ImageTexture.create_from_image(img)
...
# ---- Shadow Ranger diagonal directional sprites ----

func _gen_shadow_ranger_dir_down_side() -> void:
	# 3/4 front-diagonal — facing down-right (flip_h for down-left)
	# Hood angled, one eye visible, bow at angle
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _sr_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	# Legs
	_fill_rect(img, 12, 33, 4, 8, c["cloak_dark"])
	_fill_rect(img, 18, 34, 4, 7, c["cloak_dark"])
	# Cloak body (shifted for 3/4)
	_fill_rect(img, 9, 16, 15, 18, c["cloak_mid"])
	_fill_rect(img, 10, 17, 13, 16, c["cloak_light"])
	# Cape edge on near side
	_fill_rect(img, 5, 22, 4, 16, c["cloak_dark"])
	_fill_rect(img, 23, 22, 3, 16, c["cloak_dark"])
	_fill_rect(img, 9, 30, 15, 2, c["quiver"])
	# Hood (3/4 — one eye visible)
	_fill_rect(img, 11, 6, 11, 11, c["hood"])
	_fill_rect(img, 12, 7, 9, 9, c["cloak_dark"])
	_fill_rect(img, 17, 10, 5, 5, c["skin"])
	_fill_rect(img, 19, 11, 2, 2, c["eyes"])
	# Bow at angle (more visible)
	_fill_rect(img, 3, 12, 2, 26, c["bow_wood"])
	_fill_rect(img, 2, 10, 2, 3, c["bow_wood"])
	_fill_rect(img, 2, 37, 2, 3, c["bow_wood"])
	_fill_rect(img, 5, 12, 1, 26, c["bow_string"])
	# Quiver on back
	_fill_rect(img, 24, 14, 4, 13, c["quiver"])
	_fill_rect(img, 25, 12, 2, 3, c["bow_wood"])
	textures["shadow_ranger_dir_down_side"] = ImageTexture.create_from_image(img)

func _gen_shadow_ranger_dir_up_side() -> void:
	# 3/4 back-diagonal — facing up-right (flip_h for up-left)
	# Cape flowing to side, quiver prominent
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _sr_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 18, 34, 4, 7, c["cloak_dark"])
	# Cloak back (darker)
	_fill_rect(img, 9, 16, 15, 18, c["cloak_dark"])
	_fill_rect(img, 10, 17, 13, 16, c["cloak_mid"])
	# Cape trailing on near side
	_fill_rect(img, 5, 24, 6, 14, c["cloak_dark"])
	_fill_rect(img, 6, 25, 4, 12, Color(0.14, 0.32, 0.17))
	_fill_rect(img, 23, 22, 3, 16, c["cloak_dark"])
	_fill_rect(img, 9, 30, 15, 2, c["quiver"])
	# Hood back (3/4)
	_fill_rect(img, 11, 6, 11, 11, c["hood"])
	_fill_rect(img, 12, 7, 9, 9, c["cloak_dark"])
	# Quiver prominent on back
	_fill_rect(img, 14, 12, 7, 16, c["quiver"])
	_fill_rect(img, 15, 10, 2, 3, c["bow_wood"])
	_fill_rect(img, 19, 10, 2, 3, c["bow_wood"])
	# Bow slung over shoulder
	_fill_rect(img, 6, 14, 2, 20, c["bow_wood"])
	_fill_rect(img, 5, 12, 2, 3, c["bow_wood"])
	_fill_rect(img, 8, 14, 1, 20, c["bow_string"])
	textures["shadow_ranger_dir_up_side"] = ImageTexture.create_from_image(img)

# ---- Shadow Ranger diagonal walk cycle frames ----

func _gen_shadow_ranger_walk_down_side_1() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _sr_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 20, 33, 4, 8, c["cloak_dark"])
	# Cloak (bounce down)
	_fill_rect(img, 9, 17, 15, 18, c["cloak_mid"])
	_fill_rect(img, 10, 18, 13, 16, c["cloak_light"])
	_fill_rect(img, 5, 23, 4, 16, c["cloak_dark"])
	_fill_rect(img, 23, 23, 3, 16, c["cloak_dark"])
	_fill_rect(img, 9, 31, 15, 2, c["quiver"])
	# Hood
	_fill_rect(img, 11, 7, 11, 11, c["hood"])
	_fill_rect(img, 12, 8, 9, 9, c["cloak_dark"])
	_fill_rect(img, 17, 11, 5, 5, c["skin"])
	_fill_rect(img, 19, 12, 2, 2, c["eyes"])
	# Bow
	_fill_rect(img, 3, 13, 2, 26, c["bow_wood"])
	_fill_rect(img, 2, 11, 2, 3, c["bow_wood"])
	_fill_rect(img, 2, 38, 2, 3, c["bow_wood"])
	_fill_rect(img, 5, 13, 1, 26, c["bow_string"])
	# Quiver
	_fill_rect(img, 24, 15, 4, 13, c["quiver"])
	_fill_rect(img, 25, 13, 2, 3, c["bow_wood"])
	textures["shadow_ranger_walk_down_side_1"] = ImageTexture.create_from_image(img)

func _gen_shadow_ranger_walk_down_side_2() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _sr_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 16, 40, 5, 6, c["boot"])
	_fill_rect(img, 14, 33, 4, 8, c["cloak_dark"])
	_fill_rect(img, 16, 33, 4, 8, c["cloak_dark"])
	# Cloak (bounce up)
	_fill_rect(img, 9, 15, 15, 18, c["cloak_mid"])
	_fill_rect(img, 10, 16, 13, 16, c["cloak_light"])
	_fill_rect(img, 5, 21, 4, 16, c["cloak_dark"])
	_fill_rect(img, 23, 21, 3, 16, c["cloak_dark"])
	_fill_rect(img, 9, 29, 15, 2, c["quiver"])
	# Hood
	_fill_rect(img, 11, 5, 11, 11, c["hood"])
	_fill_rect(img, 12, 6, 9, 9, c["cloak_dark"])
	_fill_rect(img, 17, 9, 5, 5, c["skin"])
	_fill_rect(img, 19, 10, 2, 2, c["eyes"])
	# Bow
	_fill_rect(img, 3, 11, 2, 26, c["bow_wood"])
	_fill_rect(img, 2, 9, 2, 3, c["bow_wood"])
	_fill_rect(img, 2, 36, 2, 3, c["bow_wood"])
	_fill_rect(img, 5, 11, 1, 26, c["bow_string"])
	# Quiver
	_fill_rect(img, 24, 13, 4, 13, c["quiver"])
	_fill_rect(img, 25, 11, 2, 3, c["bow_wood"])
	textures["shadow_ranger_walk_down_side_2"] = ImageTexture.create_from_image(img)

func _gen_shadow_ranger_walk_down_side_3() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _sr_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 20, 39, 5, 6, c["boot"])
	_fill_rect(img, 9, 41, 5, 5, c["boot"])
	_fill_rect(img, 20, 32, 4, 8, c["cloak_dark"])
	_fill_rect(img, 10, 33, 4, 8, c["cloak_dark"])
	_fill_rect(img, 9, 17, 15, 18, c["cloak_mid"])
	_fill_rect(img, 10, 18, 13, 16, c["cloak_light"])
	_fill_rect(img, 5, 23, 4, 16, c["cloak_dark"])
	...
	_fill_rect(img, 24, 15, 4, 13, c["quiver"])
	_fill_rect(img, 25, 13, 2, 3, c["bow_wood"])
	textures["shadow_ranger_walk_down_side_3"] = ImageTexture.create_from_image(img)

func _gen_shadow_ranger_walk_up_side_1() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _sr_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 10, 32, 4, 8, c["cloak_dark"])
	_fill_rect(img, 20, 33, 4, 8, c["cloak_dark"])
	_fill_rect(img, 9, 17, 15, 18, c["cloak_dark"])
	_fill_rect(img, 10, 18, 13, 16, c["cloak_mid"])
	_fill_rect(img, 5, 25, 6, 14, c["cloak_dark"])
	_fill_rect(img, 6, 26, 4, 12, Color(0.14, 0.32, 0.17))
	_fill_rect(img, 23, 23, 3, 16, c["cloak_dark"])
	_fill_rect(img, 9, 31, 15, 2, c["quiver"])
	_fill_rect(img, 11, 7, 11, 11, c["hood"])
	_fill_rect(img, 12, 8, 9, 9, c["cloak_dark"])
	_fill_rect(img, 14, 13, 7, 16, c["quiver"])
	_fill_rect(img, 15, 11, 2, 3, c["bow_wood"])
	_fill_rect(img, 19, 11, 2, 3, c["bow_wood"])
	_fill_rect(img, 6, 15, 2, 20, c["bow_wood"])
	_fill_rect(img, 5, 13, 2, 3, c["bow_wood"])
	_fill_rect(img, 8, 15, 1, 20, c["bow_string"])
	textures["shadow_ranger_walk_up_side_1"] = ImageTexture.create_from_image(img)

func _gen_shadow_ranger_walk_up_side_2() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _sr_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 14, 33, 4, 8, c["cloak_dark"])
	_fill_rect(img, 16, 33, 4, 8, c["cloak_dark"])
	_fill_rect(img, 9, 15, 15, 18, c["cloak_dark"])
	_fill_rect(img, 10, 16, 13, 16, c["cloak_mid"])
	_fill_rect(img, 5, 23, 6, 14, c["cloak_dark"])
	_fill_rect(img, 6, 24, 4, 12, Color(0.14, 0.32, 0.17))
	_fill_rect(img, 23, 21, 3, 16, c["cloak_dark"])
	_fill_rect(img, 9, 29, 15, 2, c["quiver"])
	_fill_rect(img, 11, 5, 11, 11, c["hood"])
	_fill_rect(img, 12, 6, 9, 9, c["cloak_dark"])
	_fill_rect(img, 14, 11, 7, 16, c["quiver"])
	_fill_rect(img, 15, 9, 2, 3, c["bow_wood"])
	_fill_rect(img, 19, 9, 2, 3, c["bow_wood"])
	_fill_rect(img, 6, 13, 2, 20, c["bow_wood"])
	_fill_rect(img, 5, 11, 2, 3, c["bow_wood"])
	_fill_rect(img, 8, 13, 1, 20, c["bow_string"])
	textures["shadow_ranger_walk_up_side_2"] = ImageTexture.create_from_image(img)

func _gen_shadow_ranger_walk_up_side_3() -> void:
	var img = Image.create(32, 48, false, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))
	var c = _sr_colors()
	_fill_rect(img, 8, 44, 16, 4, c["shadow"])
	...
	_fill_rect(img, 5, 13, 2, 3, c["bow_wood"])
	_fill_rect(img, 8, 15, 1, 20, c["bow_string"])
	textures["shadow_ranger_walk_up_side_3"] = ImageTexture.create_from_image(img)
