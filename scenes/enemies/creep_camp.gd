...
const CAMP_TYPES = {
	...
	# --- Dungeon Crypt enemies ---
	# --- Dungeon Crypt enemies (tuned to be genuinely dangerous) ---
	"cave_snake": {
		"name": "Cave Snake",
		"level_range": [10, 12],
		"sprite_type": "cave_snake",
		"move_speed": 120.0,
		"attack_range": 28.0,
		"aggro_range": 130.0,
		"attack_damage": 28,
...
func _instantiate_enemy(pending: Dictionary) -> void:
	...
	if is_mini_boss:
		# Mini-bosses get 8x HP, 2x damage, big uniform sprite so they're unmistakable
		enemy.stats.max_hp = int(enemy.stats.max_hp * 8.0)
		enemy.stats.current_hp = enemy.stats.max_hp
		enemy.stats.attack_damage = int(enemy.stats.attack_damage * 2.0)
